...
/**
 * Tells watchers that a game's log has moved, and nothing else.
 *
 * ## It broadcasts a number, not the game
 *
 * The whole message is `{ gameId, seq }`. A sequence number says "there is something new" and
 * discloses nothing — not who moved, not what they did, and above all not what is under a face-down
 * plate. Clients then fetch through `GET /games/:id/commands?since=N` exactly as they do without a
 * socket.
 *
 * That is deliberate and worth keeping. Pushing the commands themselves would mean a second path out
 * of the server carrying game data, which would have to grow its own idea of what a given client is
 * allowed to see — and the first bug in it would leak the deck. Here there is one such path, already
 * written and already tested, and the socket is only a nudge towards it.
 *
...
export class HeadsGateway implements OnModuleDestroy {
  ...
  attach(http: Server): void {
    ...
    this.server.on('connection', (socket) => {
      ...
      // A socket that errored is gone whether or not `close` follows, and a set of dead sockets is
      // a slow leak in a process that stays up for weeks.
      socket.on('error', drop)
    })
  }

  /** Say that a game's head has moved. Called after a command is safely stored, never before. */
  moved(gameId: string, seq: number): void {
    const room = this.rooms.get(gameId)
    if (!room) return
    const message = JSON.stringify({ gameId, seq })
    for (const socket of room) {
      // OPEN. A socket mid-close throws on send, and one bad watcher must not fail the others.
